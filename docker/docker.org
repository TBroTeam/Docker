...
** Setup procedure
   ...
   #+BEGIN_SRC sh :dir /sudo::
     docker rm -f Worker_FTP_4_TBro TBro-Worker TBro Worker_DB_4_TBro Chado_DB_4_TBro Generic_CHADO
     docker rmi tbro_demo tbro_worker_ftp tbro_worker tbro_chado_generate tbro_apache greatfireball/generic_postgres_db  tbro_generic_chado_generate generic_chado_db_reload
   #+END_SRC
   ...
   #+BEGIN_SRC sh :results output silent :dir /sudo:: :var latest_TBro=latest_Tbro_4_apache() extract_worker=extract_worker() prepare_demo_set=prepare_demo_set()
...
** Preparation
   ...
   #+BEGIN_SRC sh :results output silent
     for DIRECTORY in apache_container worker_container chado_create_container generic_db_container demo_container worker_ftp_container generic_chado_create_container generic_chado_db_reload_container
     do
         if [ -e "$DIRECTORY" ]
         then
...
** Generic database image
   ...
   #+BEGIN_SRC sh :tangle ./generic_db_container/000_fix-acl.sh :shebang "#!/bin/bash"
   ...
   #+BEGIN_SRC sh :tangle ./generic_db_container/010_create_user_db.sh :shebang "#!/bin/bash"
...
** Installation of generic Chado database
   #+BEGIN_SRC sh :tangle generic_chado_create_container/generate_db.sh :shebang "#!/bin/bash"
     export CHADO_DB_NAME=${CHADO_ENV_DB_NAME:-chado}
     export CHADO_DB_USERNAME=${CHADO_ENV_DB_USER:-tbro}
     export CHADO_DB_PASSWORD=${CHADO_ENV_DB_PW:-tbro}
     export CHADO_DB_HOST=${CHADO_PORT_5432_TCP_ADDR:-localhost}
   ...
   We start from the default ubuntu container
   #+BEGIN_SRC sh :tangle generic_chado_create_container/Dockerfile
     FROM ubuntu
   #+END_SRC
...
** Load chado database image
   We start from the default postgres container
   #+BEGIN_SRC sh :tangle generic_chado_db_reload_container/Dockerfile
     FROM greatfireball/generic_postgres_db
   #+END_SRC

   #+BEGIN_SRC sh :tangle ./generic_chado_db_reload_container/020_import_chado_db.sh :shebang "#!/bin/bash"
     DB_NAME=${DB_NAME:-mydatabase}
     DB_PW=${DB_PW:-mypassword}
     DB_USER=${DB_USER:-myuser}
     echo "******IMPORTING CHADO DATABASE******"
     gosu postgres postgres --single < /tmp/2014-11-25_chado_database_complete_GO1.2.sql
     echo "******CHADO DATABASE IMPORTED******"
   #+END_SRC

   Finally I have to add a user to the database
   #+BEGIN_SRC sh :tangle generic_chado_db_reload_container/Dockerfile
     ADD 020_import_chado_db.sh /docker-entrypoint-initdb.d/
